...
def diff_raytrace(w : In[int], h : In[int], image : Out[Array[Vec3]], radius : In[float]):
    # Camera setup
    ...
    d_color : Diff[Vec3]
    d_sph: Diff[Sphere]
    d_sph.center.x.val = 0
    d_sph.center.y.val = 0
    d_sph.center.z.val = -1
    d_sph.radius.val = radius
    d_sph.radius.dval = 0
    while (y < h, max_iter := 4096):
        ...
        while (x < w, max_iter := 4096):
            ...
            d_color = d_ray_color(d_ray, d_sph)
...
def raytrace(w : In[int], h : In[int], image : Out[Array[Vec3]], radius : In[float]):
    ...
    while (y < h, max_iter := 4096):
        ...
        while (x < w, max_iter := 4096):
            ...
            image[w * y + x] = ray_color_constant(ray, sph)
